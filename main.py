...
def load_vgg(sess, vgg_path):
    ...
    #   Use tf.saved_model.loader.load to load the model and weights
		...
		# My Added code. Load model and weights & get and return tensors  
		tf.saved_model.loader.load(sess, [vgg_tag], vgg_path)	 
		
		graph = tf.get_default_graph();			# get a handle to access 
...
def layers(vgg_layer3_out, vgg_layer4_out, vgg_layer7_out, num_classes):
   ...
   # TODO: Implement function
		...
		conv_L7 = tf.layers.conv2d(vgg_layer7_out, num_classes, 1, padding='same', 
																...
																kernel_regularizer = \
																		tf.contrib.layers.l2_regularizer(1e-03))
 
		#Upsample
		conv_L4_1 = \
				tf.layers.conv2d_transpose(conv_L7, num_classes, 4,
           		                     strides= (2, 2), 
		           		                 padding= 'same', 
                      		         kernel_initializer= \
																		tf.random_normal_initializer(stddev=0.01),
		...
		conv_L4_2 = tf.layers.conv2d(vgg_layer4_out, num_classes, 1, 
                                   padding= 'same', 
                                   kernel_initializer= \
																			tf.random_normal_initializer(stddev=0.01), 
                                   kernel_regularizer= \
	  ...
	  # skip connection (element-wise addition)
		layer4_out = tf.add(conv_L4_1, conv_L4_2)
 
	  # upsample
		conv_L3_1 = \
				tf.layers.conv2d_transpose(layer4_out, num_classes, 4,  
                                   strides= (2, 2), 
                                   padding= 'same', 
                                   kernel_initializer= \
																			tf.random_normal_initializer(stddev=0.01), 
    ...
    # 1x1 convolution of vgg layer 3
		conv_L3_2 = tf.layers.conv2d(vgg_layer3_out, num_classes, 1, 
                                   padding= 'same', 
                                   kernel_initializer= \
																			tf.random_normal_initializer(stddev=0.01), 
                                   kernel_regularizer= \
   	...
   	# skip connection 
		layer3a_out = tf.add(conv_L3_1, conv_L3_2)

    # upsample
		nn_last_layer = \
				tf.layers.conv2d_transpose(layer3a_out, num_classes, 16,  
                                   strides= (8, 8), 
                                   padding= 'same', 
                                   kernel_initializer= \
																		tf.random_normal_initializer(stddev=0.01), 
...
def optimize(nn_last_layer, correct_label, learning_rate, num_classes):
	...
	# make logits a 2D tensor where each row represents a pixel, each column a class
...
def run():
    ...
    #  https://www.cityscapes-dataset.com/
      ...
      # Create function to get batches
			get_batches_fn = \
						helper.gen_batch_function(os.path.join(data_dir, 'data_road/training'), 
																						image_shape)
...
#																					logits, keep_prob, input_image)
			...
			#==============================================================
			# let's save the network we created so I can use it on Movidius 
			# NCDK inference engine
			#===============================================================
