...
class TmpMock(object):
	...
	def __enter__(self):
		print("In TmpMock.__enter__\n")
		return getattr(self.module, self.attrib_name)

	def __exit__(self, type, value, traceback):
		print("In TmpMock.__exit__\n")
...
def test_optimize(optimize):
	...
	with tf.Session() as sess:
		...
		sess.run([train_op], 
			{correct_label: np.arange(np.prod(shape)).reshape(shape), learning_rate: 10})
...
def test_train_nn(train_nn):
	...
	print("In test_train_nn\n")
	epochs = 1
	batch_size = 2

	def get_batches_fn(batach_size_parm):
		print("In get_batches_fn\n")
...
def test_for_kitti_dataset(data_dir):
	...
	assert training_images_count == 289, \
			'Expected 289 training images, found {} images.'.format(training_images_count)
	assert training_labels_count == 289, \
			'Expected 289 training labels, found {} labels.'.format(training_labels_count)
	assert testing_images_count == 290, \
			'Expected 290 testing images, found {} images.'.format(testing_images_count)
